...
{        
  ...
  if (LegIndex == 1)
	  RFcoxa = CoxaAngle1[LegIndex];

  if (LegIndex == 3)
      LFcoxa = CoxaAngle1[LegIndex];
...
void BalanceBody(void)
{
  ...
  if (COGShiftNeeded) {
    ...
    // Add direction variable. The body will not shift in the walking direction 
    ...
    //TotalTransX = (TotalTransX*BalCOGTransX/c2DEC)+(0.5*(TotalTransZ*BalCOGTransZ/c2DEC));
    //TotalTransZ = (TotalTransZ*BalCOGTransZ/c2DEC)+(0.5*(TotalTransX*BalCOGTransX/c2DEC));
...
#define TarsOffsetY  0		//Vector value / The 2 DOF IK calcs (femur and tibia) are based upon these vectors
  ...
  TibiaAngle1[LegIKLegNr] = -(900 + CTIBIAHORNOFFSET1(LegIKLegNr) - (long)AngleRad4*180/3141);
